...
.grat-line-view {{
    ...
    border-bottom: 1px dashed #E8AEBB;
    color: #4F4447;
    font-size: 0.92rem;
    line-height: 1.55;
}}

.grat-line-view.empty {{
    color: #AFA4A8;
}}
...
.grat-edit-area div[data-testid="stTextArea"] textarea {{
    background-color: #FFFDFB !important;
    border: 1px dashed #E8AEBB !important;
...
div[data-testid="stTextArea"] textarea {{
    ...
    border: 1px dashed #e8c8d0 !important;
...
@st.cache_data
def load_habit_log_data() -> pd.DataFrame:
    response = supabase.table("habit_log").select("*").order("date", desc=True).execute()
    data = response.data if response.data else []
    df = pd.DataFrame(data)

    if df.empty:
        return pd.DataFrame(columns=["id", "habit_id", "date", "done"])
...
def load_habit_data() -> pd.DataFrame:
    response = supabase.table("habit_tracker").select("*").order("id").execute()
    data = response.data if response.data else []
    df = pd.DataFrame(data)

    if df.empty:
        return pd.DataFrame(columns=["id", "habit_name", "created_at"])

    df["habit_name"] = df["habit_name"].fillna("").astype(str)
    df["created_at"] = pd.to_datetime(df["created_at"], errors="coerce").dt.date
    return df
...
def add_habit(habit_name: str):
    ...
    supabase.table("habit_tracker").insert(payload).execute()
    st.cache_data.clear()


def delete_habit(habit_id: int):
    supabase.table("habit_tracker").delete().eq("id", habit_id).execute()
    st.cache_data.clear()
...
def update_habit_log(habit_id: int, log_date: date, value: bool):
    ...
    supabase.table("habit_log").upsert(payload, on_conflict="habit_id,date").execute()
    st.cache_data.clear()
    ...
    with st.container(border=True):
        ...
        if subset.empty:
            ...
            for row in subset.itertuples():
                c1, c2, c3, c4, c5 = st.columns([0.6, 1, 0.22, 0.22, 0.22], gap="small")
                with c1:
                    if bool(row.carry_over):
                        st.markdown(
                            f"<span style='font-size:0.9rem; color:#B56E7B; text-decoration: line-through; text-decoration-color:#D9534F; text-decoration-thickness:1px;'>{row.task_name}</span>",
                            unsafe_allow_html=True
                        )
                        st.caption("已延到下一天")
                    else:
                        st.markdown(
                            f"<div style='font-size:0.9rem; font-weight:600; line-height:1.2;'>{row.task_name}</div>",
                            unsafe_allow_html=True
                        )
...
with tab6:
    ...
    def render_grat_day_card(day_date, weekday, grat_year, grat_week):
        ...
        with st.container(border=True):
            ...
            edit_key = f"editing_grat_{grat_year}_{grat_week}_{weekday}"

            v1 = "" if pd.isna(day_data.get("gratitude_1", "")) else str(day_data.get("gratitude_1", ""))
            v2 = "" if pd.isna(day_data.get("gratitude_2", "")) else str(day_data.get("gratitude_2", ""))
            v3 = "" if pd.isna(day_data.get("gratitude_3", "")) else str(day_data.get("gratitude_3", ""))
            ...
            if not st.session_state[edit_key]:
                for text, placeholder in [
                    ...
                    display_text = text.strip() if text.strip() else placeholder
                    empty_class = " empty" if not text.strip() else ""
                    st.markdown(
                        f"""
                        <div class="grat-line-view{empty_class}">
                            <span class="grat-heart-prefix">♡</span>{display_text}
                        </div>
                        """,
                        unsafe_allow_html=True,
                    )
                ...
                if st.button(
                    "編輯 ♡",
                    key=f"edit_grat_{grat_year}_{grat_week}_{weekday}",
                    use_container_width=True
                ):
                ...
                st.markdown('<div class="grat-edit-area">', unsafe_allow_html=True)
                ...
                if save_col.button(
                    "儲存 ♡",
                    key=f"save_grat_{grat_year}_{grat_week}_{weekday}",
                    use_container_width=True
                ):
                    update_gratitude_day(
                        grat_year, grat_week, day_date, weekday,
                        g1, g2, g3,
                        str(st.session_state.get(f"mood_score_{grat_year}_{grat_week}_{weekday}", 0))
                    )
                    st.session_state[edit_key] = False
                ...
                if cancel_col.button(
                    "取消",
                    key=f"cancel_grat_{grat_year}_{grat_week}_{weekday}",
                    use_container_width=True
                ):
                    st.session_state[edit_key] = False
                    st.rerun()
    ...
    with side_col:
        ...
        with st.container(border=True):
            st.markdown(
                """
                <div class="memo-title">This Week’s Little Joys ✨</div>
                <div class="memo-subtitle">本週小亮點回顧</div>
                """,
                unsafe_allow_html=True
            )
        ...
        with st.container(border=True):
            st.markdown(
                """
                <div class="memo-title">自由書寫區 Free Notes</div>
                <div class="memo-subtitle">想說的話、心情、靈感放這裡吧。</div>
                """,
                unsafe_allow_html=True
            )
